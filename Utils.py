...
def read_label(label_filename):
    loaded_data = torch.load('/home/alejandro-fernandez/ros2_ws/src/OpenPCDet/tools/datos3.pt',map_location=torch.device('cpu'))
    loaded_ref_boxes = loaded_data['ref_boxes'] #(n,7)
    loaded_ref_boxes_np = loaded_ref_boxes.cpu().numpy()
    #objects = [fu.Object3D(ref_box) for i,ref_box in enumerate(loaded_ref_boxes_np)]
    objects = [fu.Object3D(loaded_ref_boxes_np[0,:])]
